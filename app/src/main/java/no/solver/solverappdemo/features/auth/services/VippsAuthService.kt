...
/**
 * Vipps OAuth 2.0 authentication service.
 * Handles Vipps login flow matching the iOS VippsAuthProvider.swift implementation.
 *
 * Authentication Flow:
 * 1. OAuth Authorization → Opens Vipps app/web view via Custom Tabs
...
class VippsAuthService @Inject constructor(
    ...
    companion object {
        ...
        // Redirect URI - must match what's registered with Vipps
    ...
    // Pending auth continuation for when deep link callback arrives
    ...
    fun buildAuthorizationUrl(): Uri {
        ...
        return Uri.parse(VIPPS_AUTH_URL).buildUpon()
            .appendQueryParameter("client_id", getClientId())
            .appendQueryParameter("response_type", "code")
            .appendQueryParameter("scope", scopeString)
            .appendQueryParameter("state", state)
            .appendQueryParameter("redirect_uri", REDIRECT_URI)
            .appendQueryParameter("requested_flow", "app_to_app")
            .appendQueryParameter("app_callback_uri", REDIRECT_URI)
            .build()
    }

    /**
     * Start Vipps sign-in flow.
     * The caller should launch a Custom Tab with the authorization URL.
     * When the deep link callback is received, call handleAuthCallback().
     */
    suspend fun signIn(activity: Activity, launchAuth: (Uri) -> Unit): AuthTokens {
        Log.d(TAG, "Starting Vipps authentication flow")

        // Step 1: Get authorization code via OAuth flow
        val code = suspendCancellableCoroutine { continuation ->
            ...
            Log.d(TAG, "🔗 Vipps Auth URL: $authUrl")
            launchAuth(authUrl)
        }
     ...
     * Called by MainActivity when receiving solverapp://oauth/vipps?code=...
     */
    fun handleAuthCallback(uri: Uri) {
        Log.d(TAG, "Handling Vipps auth callback: $uri")
        ...
        when {
            !code.isNullOrBlank() -> {
                ...
                pendingAuthContinuation?.invoke(Result.success(code))
            }
            !error.isNullOrBlank() -> {
                val message = errorDescription ?: error
                Log.e(TAG, "OAuth error: $message")
                pendingAuthContinuation?.invoke(Result.failure(VippsAuthException(message)))
            }
            else -> {
                Log.e(TAG, "No code or error in callback")
                pendingAuthContinuation?.invoke(Result.failure(VippsAuthException("Invalid callback")))
            }
        }
        pendingAuthContinuation = null
    }
    ...
    /**
     * Step 2: Exchange authorization code for Vipps tokens
     */
